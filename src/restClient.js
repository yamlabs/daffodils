    ...
    const convertHTTPResponseToREST = (response, type, resource, params) => {
        ...
        switch (type) {
            ...
            case GET_MANY_REFERENCE:
                ...
                if(resource == "itemLocation"){
                    for (var i=0; i<json.length;i++){
                        var stockObj = json[i];
                        var remaining = (stockObj.itemssold ) + "/" + stockObj.origquantity; 
                        var earnings = (stockObj.retailprice-stockObj.origprice) * (stockObj.itemssold);    
                        stockObj.origprice  = "₱" + stockObj.origprice; 
                        stockObj.remaining = remaining;
                        stockObj.earnings = "₱" +earnings;
                        stockObj.retail_price = "₱"+stockObj.retailprice;
                    ...
                    for (var i=0; i<json.length;i++){
                        var stockObj = json[i];
                        stockObj.base_price = "₱"+stockObj.base_price;
                    }
                }
                return {
                    data: json,
